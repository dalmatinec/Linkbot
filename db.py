...
def init_db():
    ...
    cur.execute('INSERT OR IGNORE INTO admins (user_id, username, role, created_at) VALUES (?, ?, ?, ?)',
...
def register_user(user_id: int, username: str, first_name: str):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('INSERT OR IGNORE INTO users (user_id, username, first_name, joined_at, last_activity) VALUES (?, ?, ?, ?, ?)',
...
def can_get_link(user_id: int, category: str):
...
def add_admin(user_id: int, username: str, role: str = 'admin', admin_id: int = None):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('INSERT OR REPLACE INTO admins (user_id, username, role, created_at) VALUES (?, ?, ?, ?)',
...
def add_admin_log(admin_id: int, action: str, target: str = ''):
    conn = get_db()
    cur = conn.cursor()
    cur.execute('INSERT INTO admin_logs (admin_id, action, target, created_at) VALUES (?, ?, ?, ?)',
...
def add_broadcast_log(admin_id: int, success_count: int, failed_count: int) -> int:
    conn = get_db()
    cur = conn.cursor()
    cur.execute('INSERT INTO broadcasts (admin_id, success_count, failed_count, created_at) VALUES (?, ?, ?, ?)',
...
def get_all_users() -> List[Dict]:
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT user_id, banned FROM users ORDER BY joined_at DESC')
    rows = cur.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def get_recent_logs(limit: int = 30) -> List[Dict]:
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT admin_id as user_id, action, target, created_at FROM admin_logs ORDER BY created_at DESC LIMIT ?', (limit,))
    rows = cur.fetchall()
    conn.close()
    return [dict(row) for row in rows]